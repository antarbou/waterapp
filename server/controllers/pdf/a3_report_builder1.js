...
const printTextAra = (doc, size, str, x, w, y, h) => {
    doc.save()
      .fontSize(size)
        .text(' ' + lgge.ara(str), x, y, { width: w, align: 'right' });
...
const printSum = (doc, context) => {
    ...
    if (!context.isFirstPage) {
        ...
        context.sum.map((item, i) => {
            //console.log('item=',item,'_x[',i,'],=',_x[i],'context.isFirstPage=',context.isFirstPage)
    ...
    printTextAra(doc, 10, 'context.sumLabel', _x[17], 100, context.topPage, 21);
