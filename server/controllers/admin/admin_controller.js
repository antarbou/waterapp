...
exports.homepage = async (req, res) => {
    try {
        ...
        if (token) {
            ...
            console.log('[*]admin router {homepage} redirecting to [@/home]');
...
exports.signin = async (req, res) => {
    try {
        ...
        if (!user) {
            console.log('[*] Sign in page: user [', username, '] not found in db');
            return res.status(401).json({ message: 'invalid credentials' });
        }
        console.log('[*] Sign in page: user [', username, '] is found in db');
